...
json_file = open('(0, 16)model11.json','r')
json_model = json_file.read()
net = model_from_json(json_model)
net.load_weights('(0, 16)weight11.h5')
...
def generate_data(wrong_bit,num):
    ...
    Round=11
    diff=(0x0,0x10)
    wrong = np.array(range(2**24), dtype=np.uint32)
    ...
    sk24=subkeys&0x00ffffff
    wrong_sk24=sk24^w
    
    score=[]
    for delta in range(2**8):
        start1=time()
        sk_Candidate=(delta<<24)^wrong_sk24
    ...
    guess_8bit=np.array([np.where(score[i]==flag[i])[0][0] for i in range(num)],dtype=np.uint32)
    guess_key=wrong_sk24^(guess_8bit<<24)
...
num=300# Test 300 times
for wrong_bit in range(2,6):     
